...
assert len(result_1["validation"]) == 2
assert len(result_1["testing"]) == 2

assert (
  len(result_1["training"])
  + len(result_1["validation"])
...
batches = Batch(seed=42).create_batches(dataset, batch_size=3)
...
batches_1 = Batch(seed=42).create_batches(dataset, batch_size=3)
batches_2 = Batch(seed=42).create_batches(dataset, batch_size=3)
...
try:
  Batch(seed=42).create_batches(dataset, batch_size=0)
...
try:
  Batch(seed=42).split_dataset(
    dataset,
    training_ratio=0.0,   # gk boleh kosong!
...
try:
  Batch(seed=42).create_batches(dataset, batch_size="3")
except Exception as error:
  print(f"Pesan Error: {error}")
finally:
  print()
